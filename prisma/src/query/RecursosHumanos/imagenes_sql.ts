interface IQueryImagen {
    IQueryImagen(fecha_ini: string, fecha_fin: string): string;
}

export const getQueryImagen: IQueryImagen['IQueryImagen'] =
    (fecha_ini, fecha_fin) => `
--Imagenes
select
  cons."FECHA ATENCION",
...
from
  (
    SELECT
      ...
      DVS.docvntsunat_usrcreacion AS "USER"
    FROM
      ...
      INNER JOIN sch_clinica.tbl_segus S ON (
        ...
        AND (
          S.segus_area_cod = 31
          OR S.segus_area_cod = 51
          OR S.segus_area_cod = 52
          or S.segus_area_cod = 57
          or S.segus_area_cod = 58
        )
      )
        ...
        FROM
          ...
          INNER JOIN (
            ...
            FROM
              sch_clinica.tbl_emergencia_consulta
            GROUP BY
              emergencia_consulta_oacod
          ) EMG ON (
            emergencia_consulta_oacod = EMG.codigo
            AND emergencia_consulta_oaitem = EMG.item
          )
      ) EMGC ON (OA.ordatencion_cod = EMGC.codigo)
      LEFT JOIN sch_clinica.vw_persona med ON (
        COALESCE(EMGC.pacod, se.servicio_ejecucion_pacod) = med.vw_persona_cod
      )
      LEFT JOIN sch_clinica.tbl_espprofatencion esp ON (
        COALESCE(EMGC.emcod, cm.citamedica_emcod) = esp.espprofatencion_cod
      )
    ...
    WHERE
      ...
      AND DATE(DVS.docvntsunat_femision) <= '${fecha_fin}' and
    ...
    select
      ...
      coalesce(
        DET.valor_neto,
        DVSD.docvntsunatsubdet_cantidad * case
          when dvs.docvntsunat_oa_codigo is null then dvsd.docvntsunatsubdet_pu_neto
          else DVSD.docvntsunatsubdet_pu_venta
        end
      ) as "VALOR VENTA",
    ...
    from
      sch_clinica.tbl_docvntsunat dvs
      inner join sch_clinica.tbl_docvntsunatdet dvd on (
        dvs.docvntsunat_cod = dvd.docvntsunatdet_cod
        and dvd.docvntsunatdet_tpelemento = 1
        and (
          dvd.docvntsunatdet_elemento_item = 31
          OR dvd.docvntsunatdet_elemento_item = 51
          OR dvd.docvntsunatdet_elemento_item = 52
          or dvd.docvntsunatdet_elemento_item = 57
          or dvd.docvntsunatdet_elemento_item = 58
        )
      )
      inner join sch_clinica.tbl_docvntsunatsubdet dvsd on (
        dvd.docvntsunatdet_cod = dvsd.docvntsunatsubdet_sv_cod
        and dvd.docvntsunatdet_item = dvsd.docvntsunatsubdet_sv_item
      )
      LEFT JOIN sch_clinica.tbl_tarifariodet TD ON (
        cast(DVSD.docvntsunatsubdet_elemento_cod as integer) = TD.tarifariodet_cod
        AND cast(DVSD.docvntsunatsubdet_elemento_item as integer) = TD.tarifariodet_item
      )
      left JOIN sch_clinica.tbl_segus S ON (
        td.tarifariodet_cod = S.segus_tarifariodet_cod
      ...
      LEFT JOIN sch_clinica.tbl_tppaciente TP ON (OA.ordatencion_tpaciente_cod = TP.tppaciente_cod)
      LEFT JOIN sch_clinica.tbl_ordatencion_factura_historial OAFH ON (
        oa.ordatencion_cod = oafh.ordatencion_factura_historial_oacod
        AND OAFH.ordatencion_factura_historial_estado = 1
        and oafh.ordatencion_factura_historial_tipo = 1
        and oafh.ordatencion_factura_historial_preventa is null
      )
      left join sch_clinica.tbl_empresa aseg on (
        oa.ordatencion_negprc_emp_aseg_acod = aseg.empresa_cod
      )
      ...
      left join (
        ...
        FROM
          ...
          left join sch_clinica.tbl_segus on (
            sch_clinica.tbl_recetamedsegus.recetamedsegus_segus_cod = sch_clinica.tbl_segus.segus_tarifariodet_cod
            and sch_clinica.tbl_recetamedsegus.recetamedsegus_segus_item = sch_clinica.tbl_segus.segus_tarifariodet_item
          )
          INNER JOIN sch_clinica.tbl_rmsegus_prvnt on (
            sch_clinica.tbl_recetamedsegus.recetamedsegus_cod = sch_clinica.tbl_rmsegus_prvnt.rmsegus_prvnt_rscod
          )
          INNER JOIN sch_clinica.tbl_servicio_ejecucion ON (
            sch_clinica.tbl_rmsegus_prvnt.rmsegus_prvnt_prvntcod = sch_clinica.tbl_servicio_ejecucion.servicio_ejecucion_predocvnt_cod
          ...
          INNER JOIN sch_clinica.tbl_predocvntdet on (
            sch_clinica.tbl_rmsegus_prvnt.rmsegus_prvnt_prvntcod = sch_clinica.tbl_predocvntdet.predocvntdet_cod
            and sch_clinica.tbl_rmsegus_prvnt.rmsegus_prvnt_prvntitem = sch_clinica.tbl_predocvntdet.predocvntdet_item
          )
          INNER JOIN sch_clinica.tbl_docvntsunat ON (
            sch_clinica.tbl_rmsegus_prvnt.rmsegus_prvnt_prvntcod = sch_clinica.tbl_docvntsunat.docvntsunat_predocvnt_cod
          )
          INNER JOIN sch_clinica.tbl_ordatencionasegdet ON (
            sch_clinica.tbl_recetamed.recetamed_ordatenciondet_cod = sch_clinica.tbl_ordatencionasegdet.ordatencionasegdet_ordatencion_cod
          )
        ...
        GROUP BY
          ...
          sch_clinica.tbl_segus.segus_tpejecucion,
          sch_clinica.tbl_servicio_ejecucion.servicio_ejecucion_cantidad,
          sch_clinica.tbl_docvntsunat.docvntsunat_cod
      ) as det on (
        DET.oa_cod = OA.ordatencion_cod
        AND DET.elemento_cod = TD.tarifariodet_cod
        AND DET.elemento_item = TD.tarifariodet_item
      ...
      left join sch_clinica.tbl_servicio_ejecucion se on (
        RSP.rmsegus_prvnt_prvntcod = se.servicio_ejecucion_predocvnt_cod
        and RSP.rmsegus_prvnt_prvntitem = se.servicio_ejecucion_predocvnt_item
      )
      left join sch_clinica.tbl_ordenatencionautoseg oaaut on (
        oa.ordatencion_cod = oaaut.ordenatencionautoseg_cod
      )
      ...
      left join sch_clinica.tbl_ordatencionamb oaa on oa.ordatencion_cod = oaa.ordatencionamb_ordatenciondet_cod
      left join sch_clinica.tbl_citamedica cm on oaa.ordatencionamb_citamedica_cod = cm.citamedica_cod
      left join sch_clinica.tbl_emergencia_consulta ec on rm.recetamed_consulta_oacod = ec.emergencia_consulta_oacod
      and rm.recetamed_consulta_oaitem = ec.emergencia_consulta_oaitem
      LEFT JOIN (
        SELECT
          DISTINCT emergencia_consulta_oacod AS codigo,
          emergencia_consulta_emcod AS emcod,
      ...
      LEFT JOIN sch_clinica.tbl_persona med ON (
        COALESCE(EMGC.pacod, se.servicio_ejecucion_pacod) = med.persona_cod
      )
      ...
      left join sch_clinica.tbl_hospitalizacion hosp on oa.ordatencion_cod = hosp.hospitalizacion_procedencia_oacod
      and tp.tppaciente_cod = 2
      LEFT JOIN sch_clinica.tbl_empresa EMP ON (
        OA.ordatencion_negprc_emp_aseg_ecod = EMP.empresa_cod
      )
    where
      DATE(DVS.docvntsunat_femision) between date(
        '${fecha_ini}') and date('${fecha_fin}')
      ...
      DVS2.docvntsunat_usrcreacion AS "USER"
        FROM
          ...
          left join sch_clinica.tbl_espprofatencion esp on coalesce(
            h.hospitalizacion_emcod,
            qui.quirofano_profatencion_emcod
          ) = esp.espprofatencion_cod
          LEFT JOIN sch_clinica.tbl_empresa EMP ON (
            OA.ordatencion_negprc_emp_aseg_ecod = EMP.empresa_cod
          )
        WHERE
