...
SELECT DISTINCT
  ...
  coalesce(DET.valor_neto,DVSD.docvntsunatdet_cantidad * 
  case when dvs.docvntsunat_oa_codigo is null then dvsd.docvntsunatdet_preciounineto else DVSD.docvntsunatdet_preciouniventa end) as "VALOR VENTA",
...
FROM
  ...
  AND DVSD.docvntsunatdet_elemento_item = S.segus_tarifariodet_item AND (S.segus_area_cod = 49 or S.segus_area_cod = 53))
  ...
  (dvs.docvntsunat_cod = se.servicio_ejecucion_docvntsunatdet_cod and
   dvsd.docvntsunatdet_item = se.servicio_ejecucion_docvntsunatdet_item and
    se.servicio_ejecucion_ejecucion_auditoria = 1)
  ...
  LEFT JOIN sch_clinica.vw_persona med ON (COALESCE(EMGC.pacod, cm.citamedica_pacod, se.servicio_ejecucion_pacod) = med.vw_persona_cod)
...
--Emergencia
  ...
  EMP.empresa_rznsocial AS empresa
      from
        ...
        and dvd.docvntsunatdet_tpelemento = 1 and dvd.docvntsunatdet_elemento_item = 49)
...
select
   ...
   LEFT JOIN sch_clinica.tbl_espprofatencion esp ON (COALESCE(h.hospitalizacion_emcod, qui.quirofano_profatencion_emcod) = esp.espprofatencion_cod)
   left join sch_clinica.tbl_tarifariodet td on (oiss.tdcod = td.tarifariodet_cod
   and oiss.tditem = td.tarifariodet_item)
   left join sch_clinica.tbl_catgenerico cgc on (td.tarifariodet_cat_aux = cgc.catgenerico_cod)
   LEFT JOIN sch_clinica.tbl_empresa EMP ON (OA.ordatencion_negprc_emp_aseg_ecod = EMP.empresa_cod)
WHERE
    (OISS.cod_area = 49 or OISS.cod_area = 53) AND
