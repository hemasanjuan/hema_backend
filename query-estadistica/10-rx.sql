...
FROM
  ...
  AND DVSD.docvntsunatdet_elemento_item = S.segus_tarifariodet_item AND S.segus_area_cod = 31)
...
UNION ALL
 ...
 SELECT
  ...
  per1.persona_sexo as "SEXO",
    ...
    case when oiss.quirofano_codigo is not null then 'ATENCION QUIROFANO' else TA.tpatencion_dsc end AS tp_atencion,
    med.vw_persona_nombres,
    OI.ordinternamiento_oacod AS oa_codigo,  
    ROUND(CAST(OID.ordinternamientodet_cantidad * OID.ordinternamientodet_preciouniventa AS NUMERIC), 2),
